...
def export_notebook(store: WorkspaceStore) -> bytes:
    ...
    # Replay the session tree as a time-ordered event log. This matters when a
    # child session is created from a parent after operation A, but the parent
    # later receives operation B. The child must copy the parent's state at the
    # branch time, not the parent's final state.
