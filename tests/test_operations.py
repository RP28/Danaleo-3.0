...
def test_replace_values_supports_single_and_multiple_replacements():
    ...
    single = apply_operation(df, "replace_values", {"column": "label", "old_value": "old", "new_value": "new"})
    assert single["label"].tolist()[:2] == ["new", "new"]

    multiple = apply_operation(
        df,
        "replace_values",
        {"column": "category", "old_value": "A,B", "new_value": "Alpha,Beta", "multiple": True},
    )
    ...
    with pytest.raises(ValueError, match="same comma-separated count"):
        apply_operation(
            df,
            "replace_values",
            {"column": "category", "old_value": "A,B", "new_value": "Alpha", "multiple": True},
        )
